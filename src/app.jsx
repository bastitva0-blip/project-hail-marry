...
function AuthScreen({users,onAuthenticated}){
  const [email,setEmail]=useState('admin@kgpinovation.com');
  ...
  const signIn=e=>{
    ...
    if(!user){setError('Use one of the demo accounts below to continue.');return;}
    setError('');
  ...
  return <div className="auth-shell" style={{minHeight:'100vh',display:'grid',gridTemplateColumns:'minmax(0,1.1fr) minmax(360px,.9fr)',background:C.bg,color:C.ink}}>
    <div className="auth-hero" style={{padding:'clamp(32px,7vw,96px)',background:'linear-gradient(135deg,#263A9B 0%,#4F6EF7 54%,#13A6A1 100%)',color:'#fff',display:'flex',flexDirection:'column',justifyContent:'space-between',minHeight:'100vh'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,fontWeight:800,fontSize:19}}><span style={{width:32,height:32,borderRadius:9,display:'grid',placeItems:'center',background:'rgba(255,255,255,.18)',fontSize:17}}>⚡</span>KGP Innovation</div>
      <div style={{maxWidth:570}}>
        <div style={{fontSize:12,fontWeight:800,letterSpacing:'.12em',textTransform:'uppercase',opacity:.72,marginBottom:16}}>Unified IoT Console</div>
        <h1 style={{fontSize:'clamp(34px,5vw,64px)',lineHeight:1.02,letterSpacing:'-.03em',margin:0}}>One clear view of every connected device.</h1>
        <p style={{fontSize:16,lineHeight:1.6,opacity:.82,maxWidth:500,margin:'24px 0 0'}}>Monitor fleet health, inspect telemetry, and control your live simulation from one operational workspace.</p>
      </div>
      <div style={{fontSize:12,opacity:.7}}>Demo environment · AWS simulation mode</div>
    </div>

    <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      <div style={{width:'100%',maxWidth:410}}>
        <div style={{marginBottom:28}}><div style={{fontSize:12,color:C.blue,fontWeight:800,letterSpacing:'.08em',textTransform:'uppercase'}}>Welcome back</div><h2 style={{fontSize:30,margin:'8px 0 6px'}}>Sign in to your console</h2><p style={{color:C.soft,margin:0,lineHeight:1.5}}>This is a simulated sign-in for your product demo.</p></div>
        <form onSubmit={signIn} style={{background:'#fff',borderRadius:16,padding:24,boxShadow:'0 12px 40px rgba(17,24,39,.08)',border:`1px solid ${C.line}`}}>
          <Field label="Email"><Input value={email} onChange={e=>setEmail(e.target.value)} type="email" autoComplete="username"/></Field>
          ...
          {error&&<div style={{background:C.redBg,color:C.red,borderRadius:9,padding:'9px 12px',fontSize:12,marginBottom:14}}>{error}</div>}
          <Btn v="primary" style={{width:'100%',justifyContent:'center',padding:'11px 15px'}}>Sign in</Btn>
          <div style={{display:'flex',alignItems:'center',gap:10,margin:'20px 0 14px',color:C.faint,fontSize:11}}><span style={{height:1,background:C.line,flex:1}}/>DEMO ACCOUNTS<span style={{height:1,background:C.line,flex:1}}/></div>
          <div style={{display:'grid',gap:8}}>{users.map(user=><button type="button" key={user.id} onClick={()=>useDemo(user)} style={{display:'flex',justifyContent:'space-between',alignItems:'center',textAlign:'left',background:'#fff',border:`1px solid ${C.line}`,borderRadius:9,padding:'10px 12px',cursor:'pointer',fontFamily:'inherit'}}><span><b style={{display:'block',fontSize:12}}>{user.name}</b><span style={{fontSize:11,color:C.faint}}>{user.email}</span></span><span style={{fontSize:10,fontWeight:800,color:C.blueDark,background:C.blueLight,padding:'4px 7px',borderRadius:12}}>{user.role}</span></button>)}</div>
        </form>
        <div style={{fontSize:11,color:C.faint,textAlign:'center',marginTop:16}}>No real credentials are collected or transmitted.</div>
      </div>
    </div>
    <style>{`@media(max-width:760px){.auth-shell{display:block!important;}.auth-hero{display:none!important;}}`}</style>
  </div>;
}
...
export default function App(){
  ...
  if(!authenticated)return <AuthScreen users={users} onAuthenticated={id=>{setCurrentUser(id);setAuthenticated(true);}}/>;
    ...
    <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 26px',background:'#fff',borderBottom:`1px solid ${C.line}`,position:'sticky',top:0,zIndex:20,flexWrap:'wrap',gap:10}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
          ...
          <Btn v="outline" onClick={()=>setAuthenticated(false)}>Sign out</Btn>
